...
echo "WITH fa, fl, fp CREATE (fl)-[:ENTRY]->(cred:AccountDebit {date: date('${year}-${month}-${day}'), amount:${amount}, transactionInfo: '${transaction_info}', notes:'${notes}'}) return fa,fl,fp,cred;"